...
public class ReadTableUtils {
	...
	public List<Map<String, String>> readAllTableNames(CgDataSource dataSource) throws Exception {
		...
		try {
		    ...
		    if("mysql".equals(dataSource.getDatabaseType())) {
		    	this.sql = MessageFormat.format("select table_name,table_comment from information_schema.tables where table_schema = {0}", new Object[] { "'"+dataSource.getDatabaseName()+"'" });
		    }
		    else if("oracle".equals(dataSource.getDatabaseType())) {
		        this.sql = " select distinct colstable.table_name as  table_name from user_tab_cols colstable order by colstable.table_name asc";
		    }	
		    else if("sqlserver".equals(dataSource.getDatabaseType())) {
		        this.sql = "select distinct c.name as  table_name from sys.objects c order by c.table_name asc";
		    }
